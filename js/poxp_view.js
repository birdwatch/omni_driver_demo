...
POXP.init = function (cb, src = null, query = null) {
    const poxp = new PoxPlayer("#screen1", { capture: true, noWebGL2: false });
    POXP.poxp = poxp;
    ...
    document.querySelectorAll("#padl button").forEach((o) => {
        o.addEventListener("mouseover", (ev) => {
            mover(padl, poxp.pox.leftPad, ev);
        });
        o.addEventListener("touchstart", (ev) => {
            tstart(padl, poxp.pox.leftPad, ev);
        });
        o.addEventListener("mousedown", (ev) => {
            mdown(padl, poxp.pox.leftPad, ev);
        });
        o.addEventListener("mouseup", (ev) => {
            mup(padl, poxp.pox.leftPad, ev);
        });
        o.addEventListener("touchend", (ev) => {
            tend(padl, poxp.pox.leftPad, true, ev);
        });
        o.addEventListener("mouseout", (ev) => {
            clear(padl, poxp.pox.leftPad, true, ev);
        });
        o.addEventListener("touchcancel", (ev) => {
            clear(padl, poxp.pox.leftPad, true, ev);
        });
    });
    ...
    document.querySelectorAll("#padr button").forEach((o) => {
        o.addEventListener("mouseover", (ev) => {
            mover(padr, poxp.pox.rightPad, ev);
        });
        o.addEventListener("touchstart", (ev) => {
            tstart(padr, poxp.pox.rightPad, ev);
        });
        o.addEventListener("mousedown", (ev) => {
            mdown(padr, poxp.pox.rightPad, ev);
        });
        o.addEventListener("mouseup", (ev) => {
            mup(padr, poxp.pox.rightPad, ev);
        });
        o.addEventListener("touchend", (ev) => {
            tend(padr, poxp.pox.rightPad, false, ev);
        });
        o.addEventListener("mouseout", (ev) => {
            clear(padr, poxp.pox.rightPad, false, ev);
        });
        o.addEventListener("touchcancel", (ev) => {
            clear(padr, poxp.pox.rightPad, false, ev);
        });
    ...
    return new Promise((resolve, reject) => {
        POXP.poxp
            ...
            .then((pox) => {
                ...
                if (POXP.setting.copyright)
                    $("footer").innerHTML = POXP.setting.copyright;
                resolve(pox);
            })
            .catch((err) => {
                console.log(err);
...
POXP.msg = (msg) => {
    ...
    if (typeof msg == "string")
        msg = msg
            .replace(/\(http.*?\)/g, "")
            .replace(/at PoxPlayer\.\w+/g, "")
            .replace(/at Object\.\w+/g, "")
            .replace(/at Object\.POX\.\w+/g, "")
            .replace(/at new Function \(.+?\)/g, "");
    e.innerHTML = msg;
    $("msgc").appendChild(e);
